...
class GhostConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, act=True):
        ...
        self.primary_conv = nn.Sequential(
            nn.Conv2d(c1, c_, k, s, padding=k//2, bias=False),
            nn.BatchNorm2d(c_),
    ...
    def forward(self, x):
        ...
        return torch.cat([y, self.cheap_conv(y)], 1)
...
# 🔹 C3MAF
class C3MAF(nn.Module):
    def __init__(self, c1, c2, shortcut=True, groups=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # channel bottleneck
        self.cv1 = GhostConv(c1, c_, 1, 1)
        self.cv2 = GhostConv(c_, c_, 3, 1)
        self.dwconv = nn.Conv2d(c_, c_, 3, 1, 1, groups=c_, bias=False)
        self.attn = ECAAttention(c_)
        self.cv3 = nn.Conv2d(c_ + c_, c2, 1, 1)
...
class C3FA(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(y1)
        y2 = self.dwconv(y2)
        y2 = self.act(self.bn(y2))
        y2 = self.attn(y2)
        y2 = self.scale(y2)
        y = torch.cat([y1, y2], 1)
        y = self.bn2(self.cv3(y))
        return x + y if self.add else y